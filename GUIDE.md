...
Project build/validation is intentionally driven by:

- VS Code tasks (Build targets, MWCS commandlets)
- UE commandlets (MWCS create/validate widgets)

Plugin-only static verification scripts live under `Plugins/P_MiniFootball/DevTools/scripts/`.

### Plugin verification (optional)

```powershell
PowerShell -ExecutionPolicy Bypass -File Plugins/P_MiniFootball/DevTools/scripts/Verify_CodePatterns.ps1
PowerShell -ExecutionPolicy Bypass -File Plugins/P_MiniFootball/DevTools/scripts/Verify_ActionNameParity.ps1
```
